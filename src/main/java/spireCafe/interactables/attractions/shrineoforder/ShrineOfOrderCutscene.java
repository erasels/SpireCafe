    ...
    protected void onClick() {
        if (dialogueIndex == 0) {
            this.dialog.addDialogOption(OPTIONS[0]).setOptionResult((i) -> {
                goToDialogue(1);
                this.dialog.addDialogOption(OPTIONS[1] + FontHelper.colorString(OPTIONS[2], "y")).setOptionResult((j) -> {
                    goToDialogue(2);
                ...
                this.dialog.addDialogOption(OPTIONS[4] + FontHelper.colorString(OPTIONS[5], "y")).setOptionResult((j) -> {
                    goToDialogue(2);
                    character.alreadyPerformedTransaction = true;
                    AbstractDungeon.effectList.add(new BorderFlashEffect(Color.WHITE));
                    OrderDeckForRarity(AbstractCard.CardRarity.UNCOMMON);
                });
                this.dialog.addDialogOption(OPTIONS[6] + FontHelper.colorString(OPTIONS[7], "y")).setOptionResult((j) -> {
                    goToDialogue(2);
                    character.alreadyPerformedTransaction = true;
                    AbstractDungeon.effectList.add(new BorderFlashEffect(Color.WHITE));
                    OrderDeckForRarity(AbstractCard.CardRarity.RARE);
                });
                this.dialog.addDialogOption(OPTIONS[8] + FontHelper.colorString(OPTIONS[9], "y")).setOptionResult((j) -> {
                    goToDialogue(5);
                    character.alreadyPerformedTransaction = true;
                    AbstractDungeon.effectList.add(new BorderFlashEffect(Color.WHITE));
                    OrderOneBasic();
