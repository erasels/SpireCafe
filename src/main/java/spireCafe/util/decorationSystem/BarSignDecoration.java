...
public class BarSignDecoration extends Decoration{
    ...
    public void render(SpriteBatch sb) {
        super.render(sb);
        FontHelper.renderFontCentered(sb, FontHelper.buttonLabelFont, ((CafeRoom)AbstractDungeon.getCurrRoom().event).bartender.getLabelText(), x + ((img.getWidth() * Settings.scale) /2f), y + ((img.getHeight()/4f) * Settings.scale), Settings.CREAM_COLOR);
    }
}
